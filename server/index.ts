...
const clientErrorRequests = new Map<string, { count: number; resetTime: number }>();
...
function checkClientErrorRateLimit(ip: string): { allowed: boolean; retryAfter?: number } {
  const now = Date.now();
  const entry = clientErrorRequests.get(ip);

  if (!entry || now > entry.resetTime) {
    clientErrorRequests.set(ip, { count: 1, resetTime: now + 60000 });
    return { allowed: true };
  }
...
function buildErrorFingerprint(message: string, stack: string, componentStack: string): string {
...
// Client runtime error telemetry (frontend crash reports)
app.post('/api/client-error', async (req, res) => {
  const requestIp = getRequestIp(req);
  const rateCheck = checkClientErrorRateLimit(requestIp);
  ...
  try {
    ...
    const fingerprint = buildErrorFingerprint(message, stack, componentStack);
    ...
    console.error('Client telemetry route error:', error);
