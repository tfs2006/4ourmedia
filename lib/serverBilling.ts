...
export async function verifyAuthenticatedUser(headers: Record<string, any>): Promise<VerifiedUser | null> {
...
export async function consumeUserCredits(userId: string, amount: number): Promise<CreditMutationResult> {
  const supabase = getSupabaseAdmin();
  ...
  if (amount <= 1) {
    ...
    return {
      ...
      remaining: typedData?.remaining ?? 0,
    };
  }
