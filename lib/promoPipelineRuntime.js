...
function normalizeConversionPreset(preset) {
  if (!preset) return 'auto';
  const allowed = ['auto', 'fomo', 'social-proof', 'premium-authority', 'problem-solution'];
...
function buildPresetImageDirection(preset) {
  if (preset === 'auto') {
...
export async function generatePromoAnalysis(ai, url, platform = 'instagram', conversionPreset = 'auto') {
...
export async function generatePromoImage(ai, input) {
...
export async function generatePromoAsset(ai, url, platform = 'instagram', conversionPreset = 'auto') {
